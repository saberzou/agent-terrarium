...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    // Upload a 1x1 white pixel as placeholder first
    ...
    Promise.all(
      this.items.map(
        item =>
          new Promise(resolve => {
            ...
            img.onerror = () => {
              console.warn('InfiniteMenu: failed to load', item.image);
              resolve(null);
            };
            img.src = item.image;
          })
      ...
      console.log(`InfiniteMenu: ${loaded.length}/${images.length} images loaded, atlas ${this.atlasSize}x${this.atlasSize}`);
      ...
      console.log('InfiniteMenu: texture atlas uploaded');
    }).catch(err => {
      console.error('InfiniteMenu: texture init failed', err);
    });
  }
